-- =====================================================
-- CLIENT PORTAL SYSTEM - DATABASE SCHEMA (COMPATIBLE)
-- =====================================================
-- Compatible with existing user_role enum
-- Maps: admin=admin, vet/assistant/receptionist=staff, client=client

-- =====================================================
-- 1. EXTEND EXISTING user_role ENUM
-- =====================================================
...
-- =====================================================
-- 2. PROFILES TABLE (User metadata and roles)
-- =====================================================
...
-- =====================================================
-- 3. ADD user_id TO CLIENTS TABLE
-- =====================================================
DO $$ 
BEGIN
...
-- =====================================================
-- 4. ENABLE RLS (only on existing tables)
-- =====================================================
...
RETURNS user_role AS $$
    SELECT COALESCE(role, 'client'::user_role) FROM profiles WHERE id = user_id LIMIT 1;
...
-- =====================================================
-- 8. RLS POLICIES - CLIENTS (if table exists)
-- =====================================================
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'clients') THEN
...
-- =====================================================
-- 9. RLS POLICIES - PATIENTS (if table exists)
-- =====================================================
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'patients') THEN
        ...
        TO authenticated
        USING (
            owner_id IN (SELECT id FROM clients WHERE user_id = auth.uid())
...
-- =====================================================
-- 10. RLS POLICIES - APPOINTMENTS (if table exists)
-- =====================================================
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'appointments') THEN
...
-- =====================================================
-- 11. RLS POLICIES - INVOICES (if table exists)
-- =====================================================
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'invoices') THEN
...
-- =====================================================
-- 12. RLS POLICIES - COMMUNICATIONS (if table exists)
-- =====================================================
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'communications') THEN
